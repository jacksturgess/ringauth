#python filter.py [userID(s)] [param(s)] [gestureset] [device(s)] [is_displayfiltergraphs]
...
# - is_displayfiltergraphs: true or false (optional; default false; to show unfiltered/filtered graph comparison for each sensor of each gesture)
#
#opens the file <userdir>/1-cleaned/gestures-<gestureset>-<device>.csv, grabs data from a time window of <windowsize> to the end (i.e., the NFC trigger),
# and applies a low pass filter to each gesture for each <gestureset> and <device>
#outputs <userdir>/2-filtered/payment-<device>/<param>-gestures.csv or <userdir>/2-filtered/door-<device>/gestures.csv
...
t_index = 3 #gesture timestamp (in seconds, range from prewindowsize to 0)
x_index = 4 #x-value
y_index = 5 #y-value
z_index = 6 #z-value
w_index = 7 #w-value
e_index = 8 #Euclidean norm of unfiltered values
...
def filterGesture(data, name, device, gs):
	output = []
	sensors = ['Acc', 'Gyr'] if 'door' == gs and 'door' == device else ['Acc', 'Gyr', 'GRV', 'LAc']
	for s in sensors:
		times = []
		x = []
		y = []
...
if __name__ == '__main__':
	...
	for userID in userIDs:
		...
		print("FILTERING DATA: " + userID)
		
		for gs in gesturesets:
			for device in devices:
				f_infile = cleandir + 'gestures-' + gs + '-' + device + '.csv'
				if os.path.exists(f_infile):
					outdir = filterdir + gs + '-' + device + '/'
					...
					for param in params_gs:
						...
						if 'payment' == gs:
							...
							f_outfilename = outdir + rewrite_param(param) + '-gestures.csv'
						elif 'door' == gs:
							is_filter = True
							f_outfilename = outdir + 'gestures.csv'
						...
						with open(f_infile, 'r') as f:
							...
							if len(gestureindices) > 0:
								...
								f_outfile.write('GESTURE,SENSOR,GESTURE_TIMESTAMP,FILTERED_X-VALUE,FILTERED_Y-VALUE,FILTERED_Z-VALUE,FILTERED_W-VALUE,UNFILTERED_EUCLIDEAN_NORM' + filter_string)
								f_outfile.close()
								...
								for gestureindex in gestureindices:
									...
									if len(g_output) >= minsamplespergesture:
										if is_filter:
											...
											g_output = filterGesture(g_output, gestureindex, device, gs)
										else:
